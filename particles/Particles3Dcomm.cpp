...
{
  //dprintf("Entered apply_BCs_globally");
  ...
  for(int i=0; pstart < pcl_list.size(); i++)
  {
    ...
    { if(i==0)
        //dprintf("APPLY BCS_globally() - nonperiodic case");
      apply_nonperiodic_BCs_global(pcl_list, pstart);
    ...
    //if(i%10==0)
    //  dprintf("Apply_bcs_globally() - loop %i.%i.%i",i ,pstart, pcl_list.size());
...
{
  //dprintf("Entered apply_BCs_locally");
  using namespace Direction;
    ...
    switch(direction)
    {
      ...
      case XUP:
        for(int pidx=0;pidx<pcl_list.size();pidx++)
        {
          ...
          x = modulo(x, Lx, Lxinv);
          //dprintf("recved pcl#%g: remapped outside [0,%g] to x=%g", pcl.get_t(), Lx, x);
...
void Particles3Dcomm::apply_Xleft_BC(vector_SpeciesParticle& pcls, int start)
{
  int size = pcls.size();
  assert_le(0,start);
  //dprintf("Xleft apply BC Npart=%d\n",size);
  switch(bcPfaceXleft)
  {
    default:
...
void Particles3Dcomm::apply_Yleft_BC(vector_SpeciesParticle& pcls, int start)
{
  int size = pcls.size();
  assert_le(0,start);
  //dprintf("Yleft apply BC Npart=%d\n",size);
  switch(bcPfaceYleft)
  {
    default:
    ...
    case BCparticles::EXIT:
      ...
      break;
    case BCparticles::OPENBCIn:
      // dprintf("Getting in OPENBCIn-do nothing :-(");
      //pcls.resize(start);
      break;
    case BCparticles::OPENBCOut:
    /*for(int p=start;p<size;p++)
...
void Particles3Dcomm::apply_Zleft_BC(vector_SpeciesParticle& pcls, int start)
{
  int size = pcls.size();
  assert_le(0,start);
  //dprintf("Zleft apply BC Npart=%d\n",size);
  switch(bcPfaceZleft)
  {
    default:
...
void Particles3Dcomm::apply_Xrght_BC(vector_SpeciesParticle& pcls, int start)
{
  int size = pcls.size();
  assert_le(0,start);
  //dprintf("Xright apply BC Npart=%d\n",size);
...
void Particles3Dcomm::apply_Yrght_BC(vector_SpeciesParticle& pcls, int start)
{
  int size = pcls.size();
  assert_le(0,start);
  //dprintf("Yright apply BC Npart=%d\n",size);
...
void Particles3Dcomm::apply_Zrght_BC(vector_SpeciesParticle& pcls, int start)
{
  int size = pcls.size();
  assert_le(0,start);
  //dprintf("Zright apply BC Npart=%d\n",size);
