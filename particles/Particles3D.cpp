...
void Particles3D::constantVelocity(double vel, int dim, Field * EMf) {
  ...
  cout << "constantVelocity Maxwellian Distribution " << endl;
...
// #pragma omp parallel
{
  ...
  for (pidx = 0; pidx < nop; pidx++) {
	  ...
	  {
		Bxl += weights[cc] * field_components[cc][0];
		Byl += weights[cc] * field_components[cc][1];
		Bzl += weights[cc] * field_components[cc][2];
	  }
  ...
  double dx_per_pcl, double dy_per_pcl, double dz_per_pcl, int num_layers, Field * EMf)
{
  ...
  if ((i<=num_layers)*(ns==0)){
    ...
    dv0 = roty/(EMf->getRHOns(i,j,k,ns))/FourPI;
  ...
  */
  ...
  for (int kk=0; kk < npcelz; kk++)
  {
    ...
    do{
    sample_maxwellian(u,v,w,uth,vth,wth,u0+du0,v0+dv0,w0+dw0);
...
inline void Particles3D::populate_cell_with_particles_interp(int i, int j, int k, double q_per_particle,double dx_per_pcl, double dy_per_pcl, double dz_per_pcl, double rho_close[7], double V_close[3][7], int dir[6])
{
  ...
  // loop on the particles to generate in cell
  //dprintf("**populate_cell_with_particles_interp -- begin loop**"); 
  for (int ii=0; ii < npcelx; ii++)
  for (int jj=0; jj < npcely; jj++)
  for (int kk=0; kk < npcelz; kk++)
  {
    // declare new particle positon and speed
    double u,v,w,q,x,y,z;
    do{
    ...
    y = (jj + sample_u_double())*dy_per_pcl + cell_low_y;
    z = (kk + sample_u_double())*dz_per_pcl + cell_low_z;
    // compute weights to use in extrapolation routine
    double weig[3] = {0.5*(x-cell_low_x)/dx, 0.5*(y-cell_low_y)/dy, 0.5*(z-cell_low_z)/dz}; //done for linear-Taylor extrapolation  
    //dprintf("in the loop %d-%d-%d -- done positioning",ii,jj,kk);
    // compute extrapolated values n,V
    rho_til = extrapolate(rho_close,weig,dir);
    u_til   = extrapolate(V_close[0],weig,dir);
    v_til   = extrapolate(V_close[1],weig,dir);
    w_til   = extrapolate(V_close[2],weig,dir);
    //dprintf("in the loop %d-%d-%d -- done extrap",ii,jj,kk);
    //sample from maxwellian using V extrapolated
    sample_maxwellian(u,v,w,uth,vth,wth,u_til,v_til,w_til);
    // create new particle using n extrapolated
    //cout << "new density from extrapolation = " << (FourPI*rho_til) << endl;
    q = q_per_particle*FourPI*fabs(rho_til);
    }while((x>Lx) || (y>Ly) || (z>Lz) || (x*y*z)<0 || sqrt(u*u+v*v+w*w)>1.0); //avoid to create particles out of the box and supra-luminous 
    create_new_particle(u,v,w,q,x,y,z);
  }
}
...
double Particles3D::rotateAndCountParticlesInsideSphere(int cycle, double R, double x_center, double y_center, double z_center)
{
  ...
  while (pidx < _pcls.size())
  {
    ...
    if ( (xd*xd+yd*yd+zd*zd) < (R*R) ){
	 ...
	 phi   = rand() * RandNorm;
         u = Vmod*sin( theta )*cos( phi);
         v = Vmod*sin( theta )*sin( phi );
         w = Vmod*cos( theta );
  ...
  while (pidx < _pcls.size())
  {
    ...
    if ( (xd*xd+zd*zd) < (R*R) ){
      if(prm<=Nrm){
        ...
        Rnw = 2.*R - sqrt(xd*xd+zd*zd);
        xnw = Rnw/sqrt(xd*xd+zd*zd)*xd + x_center;
        znw = Rnw/sqrt(xd*xd+zd*zd)*zd + z_center;
        pcl.set_x(xnw);
        pcl.set_z(znw);
        pidx++;
      }
...
double Particles3D::AddIonizedExosphere(int i)
{
  ...
  for (int i=1; i<grid->getNXC()-1;i++)
    for (int j=1; j<grid->getNYC()-1;j++)
      for (int k=1; k<grid->getNZC()-1;k++) {
        ...
        xd = grid->getXC(i,j,k)-col->getx_center(); // x position relative to planet
        yd = grid->getYC(i,j,k)-col->gety_center(); // y position relative to planet
        zd = grid->getZC(i,j,k)-col->getz_center()-PlanetOffset; // z position relative to planet
