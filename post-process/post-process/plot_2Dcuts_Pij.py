...
for sp in from_VTK.meta['species']:
    ...
    for cycle in from_VTK.meta['segcycles']:
        ...
        PXXdp = from_VTK.get_scal(from_VTK.meta['name']+'_PXX'+sp+'dp_%i'%cycle,cycle,fibo_obj=None,tar_var='PXX')[:,0,:]
        PXXeq = from_VTK.get_scal(from_VTK.meta['name']+'_PXX'+sp+'eq_%i'%cycle,cycle,fibo_obj=None,tar_var='PXX')[:,:,0]
        PYYdp = from_VTK.get_scal(from_VTK.meta['name']+'_PYY'+sp+'dp_%i'%cycle,cycle,fibo_obj=None,tar_var='PYY')[:,0,:]
        PYYeq = from_VTK.get_scal(from_VTK.meta['name']+'_PYY'+sp+'eq_%i'%cycle,cycle,fibo_obj=None,tar_var='PYY')[:,:,0]
        PZZdp = from_VTK.get_scal(from_VTK.meta['name']+'_PZZ'+sp+'dp_%i'%cycle,cycle,fibo_obj=None,tar_var='PZZ')[:,0,:]
        PZZeq = from_VTK.get_scal(from_VTK.meta['name']+'_PZZ'+sp+'eq_%i'%cycle,cycle,fibo_obj=None,tar_var='PZZ')[:,:,0]

        # miscellanous
        fields_to_plot = [PXXdp,PYYdp,PZZdp,PXXeq,PYYeq,PZZeq]
        ...
        plt.savefig(output_path+'/plot_2Dcuts_P'+sp+'_%i.png'%cycle,format='png')
        plt.close()
