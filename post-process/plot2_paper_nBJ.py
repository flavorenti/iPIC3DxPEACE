...
for cycle in range(cycle_min,cycle_max,d_cycle):
    ...
    PeXXdp = -conv_P*from_VTK.get_scal(from_VTK.meta['name']+'_PXXe0dp_%i'%cycle,cycle,fibo_obj=None,tar_var='PXXe0')
    ...
    plt.plot(x,Pramidp[ix[0]:ix[1],0,izc],label='$P_{ram,i}$',color='orange')
    plt.hlines(0,0,10,color='grey',linestyle='--')
    plt.legend(loc=2,fontsize=16)
    ...
    plt.savefig('./images/plot2_paper_nBJ_9N_%i.png'%cycle,format='png')
    plt.close()
