...
void EMfields3D::calculateE(int cycle)
{
  ...
  if (PoissonCorrection &&  cycle%PoissonCorrectionCycle == 0) {
		...
		//if (!CG(xkrylovPoisson, (nxc - 2) * (nyc - 2) * (nzc - 2), bkrylovPoisson, 3000, CGtol, &Field::PoissonImage, this)) {
		  ...
		  //eqValue(0.0, xkrylovPoisson, (nxc - 2) * (nyc - 2) * (nzc - 2));
		  if (vct->getCartesian_rank() == 0) cout << "*** DIVERGENCE CLEANING div(E)=rho using GMRes***" << endl;
...
// modified by F.Lavorenti: it is nomore constantChargPlanet but forces fields E,B,rho to be zero inside the planet
// added DipoleOffset as well
...
void EMfields3D::calculateB()
{
  ...
  // parameters for SAL (simple absorption layer) method 
  int yes_sal;
  int n_layers;
  
  yes_sal  = col->getYes_sal();
  
  // divide two cases: apply SAL b.c., or regular zero-gradient b.c.
  if(yes_sal)
    n_layers = col->getN_layers_sal();
  else
    n_layers = 3;
  ...
  // X=0 boundary
  if(vct->getXleft_neighbor()==MPI_PROC_NULL && bcEMfaceXleft ==2) {
    for (int i_sal=0; i_sal<n_layers; i_sal++)
      for (int j=0; j < nyn; j++)
        for (int k=0; k < nzn; k++){
  ...
  if(vct->getYleft_neighbor()==MPI_PROC_NULL && bcEMfaceXleft ==2) {
    for (int i=0; i < nxn; i++)
      for (int j_sal=0; j_sal < n_layers; j_sal++)
        for (int k=0; k < nzn; k++){
  ...
  if(vct->getZleft_neighbor()==MPI_PROC_NULL && bcEMfaceXleft ==2) {
    for (int i=0; i < nxn; i++)
      for (int j=0; j < nyn; j++)
        for (int k_sal=0; k_sal < n_layers; k_sal++){
...
void EMfields3D::init()
{
  ...
  if (restart1 == 0) {
    ...
    {
      ConstantChargePlanet(col->getL_square(),col->getx_center(),col->gety_center(),col->getz_center()); // this is new - F.Lavorenti
    }
    ...
    }*/ // commented this part since I do not see its utility F. Lavorenti
    ...
    for (int j=0; j < nyn; j++){
      for (int k=0; k < nzn; k++){
        ...
        if (r2 > a*a) {
            ...
            fac1 =  -B1z*a*a/r2;  // modified F.Lavorenti
  ...
  int nx, int ny, int nz)
{
  ...
  // parameters for SAL (simple absorbing layer) method
  double sal;
  int yes_sal;
  int n_layers=3;
  ...
  //cout << "yes_sal, n_layers = " << yes_sal << "  " <<  n_layers <<endl;
...
/*! computes rot(B) at a given point of the grid */
double EMfields3D::getCurlN2CB_x(int i, int j, int k) {
	const VirtualTopology3D * vct = &get_vct();
	// curl - X
	double compZDY = .25 * (Bzn[i][j + 1][k] - Bzn[i][j][k]) / dy + .25 * (Bzn[i][j + 1][k + 1] - Bzn[i][j][k + 1]) / dy + .25 * (Bzn[i + 1][j + 1][k] - Bzn[i + 1][j][k]) / dy + .25 * (Bzn[i + 1][j + 1][k + 1] - Bzn[i + 1][j][k + 1]) / dy;
	double compYDZ = .25 * (Byn[i][j][k + 1] - Byn[i][j][k]) / dz + .25 * (Byn[i + 1][j][k + 1] - Byn[i + 1][j][k]) / dz + .25 * (Byn[i][j + 1][k + 1] - Byn[i][j + 1][k]) / dz + .25 * (Byn[i + 1][j + 1][k + 1] - Byn[i + 1][j + 1][k]) / dz; 
        return compZDY - compYDZ;
}
double EMfields3D::getCurlN2CB_y(int i, int j, int k) {
	const VirtualTopology3D * vct = &get_vct();
        // curl - Y
        double compXDZ = .25 * (Bxn[i][j][k + 1] - Bxn[i][j][k]) / dz + .25 * (Bxn[i + 1][j][k + 1] - Bxn[i + 1][j][k]) / dz + .25 * (Bxn[i][j + 1][k + 1] - Bxn[i][j + 1][k]) / dz + .25 * (Bxn[i + 1][j + 1][k + 1] - Bxn[i + 1][j + 1][k]) / dz;
        double compZDX = .25 * (Bzn[i + 1][j][k] - Bzn[i][j][k]) / dx + .25 * (Bzn[i + 1][j][k + 1] - Bzn[i][j][k + 1]) / dx + .25 * (Bzn[i + 1][j + 1][k] - Bzn[i][j + 1][k]) / dx + .25 * (Bzn[i + 1][j + 1][k + 1] - Bzn[i][j + 1][k + 1]) / dx;
	//if (vct->getCartesian_rank() == 0)
	//	cout<<"getCurlN2CB_y roty="<<compXDZ - compZDX<<endl;
	return compXDZ - compZDX;
}
double EMfields3D::getCurlN2CB_z(int i, int j, int k) {
	const VirtualTopology3D * vct = &get_vct(); 
        double compYDX = .25 * (Byn[i + 1][j][k] - Byn[i][j][k]) / dx + .25 * (Byn[i + 1][j][k + 1] - Byn[i][j][k + 1]) / dx + .25 * (Byn[i + 1][j + 1][k] - Byn[i][j + 1][k]) / dx + .25 * (Byn[i + 1][j + 1][k + 1] - Byn[i][j + 1][k + 1]) / dx;
